...
unsigned short __thiscall GdFigureGetUnitID(void * _this, unsigned short param_1)
{
    return *(unsigned int *)((unsigned int)param_1 * 0x2b3 + 0x28 + (int)_this);
}


void __declspec(naked) black_tower_hook_beta()
{
    asm("mov 0x1c(%%edi), %%ecx     \n\t"
        "push %%esi                 \n\t"
        "call %P0                   \n\t"
        "cmpw $0x8BF, %%ax          \n\t"
        "jne 1f                     \n\t"
        ...
        "1:mov -0xac(%%ebp), %%ebx  \n\t"
        "jmp *%2                    \n\t":
        :"i"(GdFigureGetUnitID), "o"(JOB_SELECT_FAIL), "o"(JOB_SELECT_EXEC));
}

void hookBetaVersion()
{
//return old code after black tower test
    /*
    ...
    JOB_SELECT_EXEC = ASI::AddrOf(0x338CA2);
    JOB_SELECT_FAIL = ASI::AddrOf(0x338D27);
    ASI::MemoryRegion mreg(ASI::AddrOf(0x338C9C), 6); //building selector

    ASI::BeginRewrite(mreg);
        *(unsigned char*)(ASI::AddrOf(0x338C9C)) = 0xE9;   // jmp instruction
        *(int*)(ASI::AddrOf(0x338C9D)) = (int)(&black_tower_hook_beta) - ASI::AddrOf(0x338CA1);
         *(unsigned char*)(ASI::AddrOf(0x338CA1)) = 0x90;
    ASI::EndRewrite(mreg);
}
